...
void * tree_search(void * i) {
	...
	while(1) {
		...
		if(local_heap_size == 0) {
			...
			int wq_id = (rand() % NUM_OPENLISTS);
			int lock_status = pthread_mutex_trylock(&hashtable_lock);
			if(lock_status != 0) {
...
int main(int argc, char ** argv) {
	...
	insert_heap_node(new, heap[0], &heap_size[0]);
